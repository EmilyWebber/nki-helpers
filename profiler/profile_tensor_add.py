...
if __name__ == "__main__":
    ...
    with torch_neuronx.experimental.profiler.profile(
        ...
        for i in range(1, num_runs):
            ...
            a = torch.ones((4, i), dtype=torch.float16).to(device=device)
            b = torch.ones((4, i), dtype=torch.float16).to(device=device)

            c = nki_tensor_add_kernel(a, b)
            
            d = torch.ones((4, i), dtype=torch.float16).to(device=device)
            e = torch.ones((4, i), dtype=torch.float16).to(device=device)

            f = nki_tensor_add_kernel(d, e)
